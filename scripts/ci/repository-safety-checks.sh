...
for x in '対象Issue' '変更内容' '検証結果' '未対応範囲' 'Handoff Summary' 'ChatGPT共有用サマリー' '本番反映を含んでいないか' 'APIキー・秘密情報を変更していないか' '顧客データ・実データを投入していないか' '対象外機能を追加していないこと'; do need "$x"; done
...
if grep -Eiq '([0-9]{2,4}-[0-9]{2,4}-[0-9]{4}|[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,})' <<<"$added"; then echo 'Potential real data detected'; fail=1; fi
exit "$fail"
